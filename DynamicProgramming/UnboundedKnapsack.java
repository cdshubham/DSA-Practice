...
public class UnboundedKnapsack {
    ...
    private int knapsackRecursiveHelper(int[] weights, int[] values, int capacity, int index) {
        ...
        // Include the current item (can be reused, so index doesn't decrease)
        int include = 0;
        if (weights[index] <= capacity) {
            include = values[index] + knapsackRecursiveHelper(weights, values, capacity - weights[index], index);
        }
    ...
    private int knapsackMemoizationHelper(int[] weights, int[] values, int capacity, int index, Integer[][] memo) {
        ...
        int include = 0;
        if (weights[index] <= capacity) {
            include = values[index] + knapsackMemoizationHelper(weights, values, capacity - weights[index], index, memo);
        }
    ...
    public int knapsackOptimized(int[] weights, int[] values, int capacity, int n) {
        ...
        for (int i = 0; i < n; i++) {
            // Update dp for all capacities from weights[i] to capacity
